...
//! loopback.
//!
//! UNVERIFIED: written against `wasapi` 0.23.0's source, never
//! compiled. This dev environment has no Windows/MSVC toolchain, and
//! even a `--target x86_64-pc-windows-gnu` type-check is blocked by
//! Opus's vendored build needing a mingw cross-toolchain that isn't
//! installed here. Validate with a real `cargo build --target
//! x86_64-pc-windows-msvc` in CI before relying on this (plano §9.1's
//! own action item — that's about Opus specifically, but applies here
//! too).
...
impl AudioCapture for WasapiCapture {
    ...
    fn start(
        ...
        let join = thread::spawn(move || {
            ...
            let run = || -> Result<AudioFormat, AudioError> {
