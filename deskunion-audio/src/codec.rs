...
impl Resampler {
    ...
    /// nudge the resample ratio relative to its starting ratio (e.g.
    /// `0.001` = +0.1%). Used for clock-drift correction — see
    /// `jitter::DriftController`. `max_resample_ratio_relative` at
    /// construction (currently 1.1) bounds how far this can move.
    pub fn set_ratio_relative(&mut self, delta: f64) -> Result<(), AudioError> {
        // rubato's `set_resample_ratio_relative` takes a *multiplier* of
        // the original ratio (e.g. 1.001), not an additive delta.
        self.inner.set_resample_ratio_relative(1.0 + delta, true)?;
        Ok(())
    }
