...
pub fn setup(app: &adw::Application, window: &adw::ApplicationWindow) {
    ...
    STATUS_ITEM.with(|item| {
        ...
        unsafe {
            ...
            let menu = menu(&[
                menu_item(c"Open Deskunion", c"showDeskunion:"),
                separator_item(),
                menu_item(c"Quit Deskunion", c"quitDeskunion:"),
            ]);
            ...
            msg_send_void_id(button, sel(c"setToolTip:"), nsstring(c"Deskunion"));
...
fn delegate_class() -> Class {
    ...
    *CLASS.get_or_init(|| unsafe {
        ...
        class_addMethod(
            class,
            sel(c"showDeskunion:"),
            show_deskunion as *const c_void,
            c"v@:@".as_ptr(),
        );
        class_addMethod(
            class,
            sel(c"quitDeskunion:"),
            quit_deskunion as *const c_void,
            c"v@:@".as_ptr(),
        );
...
extern "C" fn show_deskunion(_this: Id, _cmd: Sel, _sender: Id) {
    present_window();
}
