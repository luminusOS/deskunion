...
use deskunion_ipc::{DEFAULT_PORT, OperationMode, Position};
...
struct TomlClient {
    /// display label (legacy: dial-out hostname — the server no longer
    /// dials, connections are incoming and paired by `fingerprint`)
    hostname: Option<String>,
    host_name: Option<String>,
    /// legacy dial-out addresses; parsed for compatibility but unused
    ips: Option<Vec<IpAddr>>,
    /// legacy dial-out port; parsed for compatibility but unused
    port: Option<u16>,
    position: Option<Position>,
    activate_on_startup: Option<bool>,
    enter_hook: Option<String>,
    /// sha256 certificate fingerprint of the paired device
...
/// the capture-side endpoint this device dials in client mode
...
impl From<TomlClient> for ConfigClient {
    fn from(toml: TomlClient) -> Self {
        if toml.ips.as_ref().is_some_and(|ips| !ips.is_empty()) || toml.port.is_some() {
            log::warn!(
                "ignoring deprecated `ips`/`port` in [[clients]]: the server no longer \
                 dials out; devices connect in and are paired by `fingerprint`"
            );
        }
        ...
        let fingerprint = toml.fingerprint;
        Self {
            ips,
            hostname,
            port,
...
impl Config {
    ...
    /// Selected software-KVM role. Legacy configurations that already have
    /// outgoing clients are inferred as servers; one with a configured
    /// server endpoint is inferred as a client; a fresh install remains
    /// unconfigured so it can present the choice before requesting access.
    pub fn operation_mode(&self) -> OperationMode {
        self.config_toml
            .as_ref()
            .and_then(|config| config.operation_mode)
            .unwrap_or_else(|| {
                if self
                    .config_toml
                    .as_ref()
                    .and_then(|config| config.clients.as_ref())
                    .is_some_and(|clients| !clients.is_empty())
                {
                    ...
                    .config_toml
                    .as_ref()
                    .and_then(|config| config.server_hostname.as_ref())
                    .is_some()
                {
    ...
    /// the capture-side endpoint to dial in client mode
    pub fn server_endpoint(&self) -> ServerEndpoint {
        let toml = self.config_toml.as_ref();
        ServerEndpoint {
            hostname: toml.and_then(|c| c.server_hostname.clone()),
            ips: toml.and_then(|c| c.server_ips.clone()).unwrap_or_default(),
            port: toml.and_then(|c| c.server_port).unwrap_or(DEFAULT_PORT),
        }
    }

    /// set the server endpoint dialed in client mode
    pub fn set_server_endpoint(&mut self, endpoint: ServerEndpoint) {
        if self.config_toml.is_none() {
            self.config_toml = Some(Default::default());
        }
        let toml = self.config_toml.as_mut().expect("config");
        toml.server_hostname = endpoint.hostname;
        toml.server_ips = (!endpoint.ips.is_empty()).then_some(endpoint.ips);
        toml.server_port = (endpoint.port != DEFAULT_PORT).then_some(endpoint.port);
    }
    ...
    pub fn set_operation_mode(&mut self, mode: OperationMode) {
        ...
        self.config_toml.as_mut().expect("config").operation_mode = Some(mode);
    }
