...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    EDD_GET_DEVICE_INTERFACE_NAME, GetMessageW, HOOKPROC, KBDLLHOOKSTRUCT, LLKHF_EXTENDED, MSG,
    MSLLHOOKSTRUCT, PM_NOREMOVE, PeekMessageW, PostThreadMessageW, RegisterClassW,
...
fn try_send_event(
    ...
    EVENT_TX.with_borrow_mut(|tx| match tx.as_mut() {
        Some(tx) => tx.try_send((pos, event)),
        None => Err(TrySendError::Closed((pos, event))),
    })
}
...
fn get_msg() -> Option<MSG> {
    unsafe {
        ...
        match ret.0 {
            ...
            x if x > 0 => Some(msg),
            _ => {
                log::error!("GetMessageW failed; stopping Windows capture thread");
                None
            }
        }
    }
...
fn start_routine(
    ...
    // A thread does not own a Win32 message queue until it calls a message
    ...
    let window_class: WNDCLASSW = WNDCLASSW {
        ...
        hInstance: instance,
        lpszClassName: w!("deskunion-message-window-class"),
        ..Default::default()
    };
    ...
    unsafe {
        CreateWindowExW(
            Default::default(),
            w!("deskunion-message-window-class"),
            w!("deskunion-msg-window"),
...
fn check_client_activation(wparam: WPARAM, lparam: LPARAM) -> bool {
    ...
    let Some(active) = ACTIVE_CLIENT.get() else {
        return false;
    };
    if let Err(error) = try_send_event(active, CaptureEvent::Begin) {
        log::error!("failed to begin Windows edge handoff: {error}");
        ACTIVE_CLIENT.take();
        return false;
    }
